...
module.exports = {
	...
	IceShroom: class IceShroom extends IcebergLettuce {
		constructor(data) {
			super({name: `Ice-shroom`, sun_cost: 75, cooldown: 10, health: 0})
			Object.assign(this, data)
		}
		onDeath(action_list) {
			console.log(action_list.nearSquare(this.position, 4, `zombies`))
			action_list.nearSquare(this.position, 4, `zombies`).forEach(z => z.status.push({name: `frozen`, time: 5}))
			return new Action({ tile_render: {
				effect: `icecloud.png`,
